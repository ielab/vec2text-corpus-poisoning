...
from transformers import DPRContextEncoder, DPRContextEncoderTokenizerFast, T5PreTrainedModel
...
def get_embeddings(model):
    ...
    if isinstance(model, DPRContextEncoder):
        ...
        if isinstance(model[0].auto_model, T5PreTrainedModel):
            embeddings = model[0].auto_model.encoder.embed_tokens
        else:
...
def kmeans_split(data_dict, model, get_emb, tokenizer, k, split):
    ...
    # get query embs
    q_embs = []
    for q in tqdm(data_dict["sent0"], desc='Encoding all train queries'):
...
def main():
    ...
    valid_dataloader = DataLoader(dataset, batch_size=args.per_gpu_eval_batch_size, shuffle=False, collate_fn=lambda x: x )
    ...
    for it_ in tqdm(range(args.num_iter), desc='Iteration'):
        print('-' * 50)
        ...
        print("--- train time --- %s seconds ---" % (time.time() - train_start_time))
        ...
        print("--- eval time --- %s seconds ---" % (time.time() - eval_start_time))
        ...
        print("--- update time --- %s seconds ---" % (time.time() - update_start_time))
